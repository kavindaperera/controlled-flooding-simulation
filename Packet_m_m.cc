...
void Packet::copy(const Packet& other)
{
    ...
    delete [] this->transientNodes;
    this->transientNodes = (other.transientNodes_arraysize==0) ? nullptr : new int[other.transientNodes_arraysize];
    transientNodes_arraysize = other.transientNodes_arraysize;
    for (unsigned int i=0; i<transientNodes_arraysize; i++)
        this->transientNodes[i] = other.transientNodes[i];
}
...
void Packet::parsimPack(omnetpp::cCommBuffer *b) const
{
    ...
    doParsimArrayPacking(b,this->transientNodes,transientNodes_arraysize);
}
...
void Packet::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    if (transientNodes_arraysize==0) {
        ...
        this->transientNodes = new int[transientNodes_arraysize];
        doParsimArrayUnpacking(b,this->transientNodes,transientNodes_arraysize);
    }
}
...
::omnetpp::simtime_t Packet::getTravelTime() const
{
    return this->travelTime;
}

void Packet::setTravelTime(::omnetpp::simtime_t travelTime)
{
...
{
    int *transientNodes2 = (size==0) ? nullptr : new int[size];
    unsigned int sz = transientNodes_arraysize < size ? transientNodes_arraysize : size;
    for (unsigned int i=0; i<sz; i++)
        transientNodes2[i] = this->transientNodes[i];
    for (unsigned int i=sz; i<size; i++)
        transientNodes2[i] = 0;
    transientNodes_arraysize = size;
    delete [] this->transientNodes;
    this->transientNodes = transientNodes2;
}
...
{
    if (k>=transientNodes_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", transientNodes_arraysize, k);
    return this->transientNodes[k];
}

void Packet::setTransientNodes(unsigned int k, int transientNodes)
{
    if (k>=transientNodes_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", transientNodes_arraysize, k);
    this->transientNodes[k] = transientNodes;
}
...
std::string PacketDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    switch (field) {
        ...
        case 3: return simtime2string(pp->getTravelTime());
...
bool PacketDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    switch (field) {
        ...
        case 3: pp->setTravelTime(string2simtime(value)); return true;
